...
```{r prev graph, echo=TRUE, warning=FALSE, fig_margin=TRUE}
##prevalence by race bar graph
ggplot(data = allracesprev, aes(x = primary_race, y = analysis_value)) + geom_bar(stat="identity", fill="black") + labs(title = "Diabetes Prevalence in Elderly NC Medicare Recipients by Race", subtitle="Percentage is out of total erlderly Medicare Recipients in each racial group.", caption="Source: Mapping Medicare Disparities CMS Data") + xlab("Racial Group") + ylab("Percentage") + geom_text(aes(label=analysis_value), vjust=0.5, hjust=1.2, color="white") + coord_flip()
```

```{r allracecost, echo=TRUE}
...
```{r cost graph, echo=TRUE, warning=FALSE, fig_margin=TRUE}
##create cost bar graph
ggplot(data = allracescost, aes(x = primary_race, y = analysis_value)) + geom_bar(stat="identity", fill="black") + labs(title = "Average Cost of Diabetes-related Medicare Claims in NC by Race", subtitle="Cost is per each beneficiary for the entirety of 2019.", caption="Source: Mapping Medicare Disparities CMS Data") + xlab("Racial Group") + ylab("Cost in Dollars") + geom_text(aes(label=analysis_value), vjust=0.5, hjust=1.2, color="white") + coord_flip()
```

```{r whiteprevcountiesnc, include=FALSE}
...
```{r warning = FALSE, fig_margin = TRUE}
...
```{r blackprevcountiesnc, include=FALSE}
